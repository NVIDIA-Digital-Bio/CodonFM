...
def main():
    ...
    CURRENT_FILE_ABS_PATH = current_file = Path(__file__).resolve()
    DATA_SCRIPTS_ABS_PATH = CURRENT_FILE_ABS_PATH.parent
    ...
    PREPROCESS_VALIDATION_PATH = DATA_SCRIPTS_ABS_PATH / "preprocess_validation.py"
    for filename in files_to_process.keys():
        ...
        subprocess.run(
            [
                "python", str(PREPROCESS_VALIDATION_PATH),
                "-i", str(input_path),
                "-o", str(output_path),
